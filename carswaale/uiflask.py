...
def recommend():
    ...
    # copy['index'] = range(1, len(copy) + 1)
    # copy = copy.set_index('index')
    print("-> ",len(price),fuel,comp,driven)
    if(len(price)!=0):
        price = int(price)
    ...
    for i in range(len(copy)):
        car_name=copy.iloc[i, 0]
        car_price = copy.iloc[i, 3]
        car_brand = copy.iloc[i, 1]
        car_driven = copy.iloc[i, 4]
        car_fuel = copy.iloc[i, 5]
        car_year = copy.iloc[i, 2]
